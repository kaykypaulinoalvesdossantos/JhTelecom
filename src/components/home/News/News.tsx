...
export default function NewsPage() {
    return (
        <div className="flex flex-col justify-center items-center gap-24">
            ...
            <div className="grid grid-cols-2 grid-rows-1 gap-12 max-[1351px]:grid-cols-2 max-sm:grid-cols-1">
                {News.map((news) => (
                    <div key={news.id} className="">
                        <Link href={news.slug} className="rounded-lg  min-h-[630px] max-w-[450px] max-h-[650px] max-md:max-w-[350px] max-md:min-w-[350px] max-sm:max-w-[300px]  max-sm:min-w-[280px] max-sm:min-h-[280px] flex flex-col gap-5 max-sm:gap-2 py-4 px-4 max-lg:items-center bg-[#CDD3D9] shadow-cardNews">
                            <Image src={news.img} alt={news.title}  className="max-w-[25rem] max-h-[18rem] object-cover max-lg:max-w-[20rem] max-lg:max-h-[14rem] max-sm:max-w-full "/>
